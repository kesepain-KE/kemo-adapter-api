...
为降低高频小事务造成的磁盘 IOPS，非终态事件先进入当前响应独立的有界内存队列；默认在 50 ms、
32 条事件或终态中的任一条件达到时，以一个 SQLite 事务批量提交。缓冲只改变事务粒度，不合并或
删除 Kemo 事件，每条事件仍保留独立 `event_id` 和连续 `sequence`。不同响应的队列完全隔离。

网关严格按“先提交、后发布”执行：批次 `COMMIT` 成功后，事件才进入可重放列表并通知 SSE
客户端。因此客户端已经收到的事件必然位于数据库持久化边界内；在提交前发生进程崩溃时，尚在
内存中的事件既不会恢复，也从未发布给客户端。终态、取消和正常关停不等待 50 ms 窗口：它们会
立即 flush pending 事件，并将统一终态、最终响应和执行状态在同一事务提交。

SQLite 连接在网关生命周期内复用，sequence 热路径由内存中的已持久化序号维护；数据库的
`PRIMARY KEY (tenant_id, response_id, sequence)` 与 event ID 唯一约束仍是最终一致性防线。
若协程在 SQLite 工作线程提交期间被取消，网关会先等待已启动事务结束并同步内存重放边界，再传播
取消。持久化失败会终止等待中的订阅者，不会让流永久挂起。

持久化只保证已经提交到本地数据库的事件和响应能够重放，不代表上游推理可以跨网关进程继续。
进程异常退出时尚未提交的上游输出无法恢复，也不会在重启后偷偷重新请求 Provider。
