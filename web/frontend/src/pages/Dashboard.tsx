...
function TrendChart({ points, mode }: { points: TrendPoint[]; mode: TrendMode }) {
  ...
  let currentTokenSegment: string[] = []
  points.forEach((point, index) => {
    if (point.tokens == null) {
      if (currentTokenSegment.length) tokenSegments.push(currentTokenSegment.join(' '))
      currentTokenSegment = []
    } else currentTokenSegment.push(`${x(index)},${y(point.tokens, tokenMax)}`)
  })
  if (currentTokenSegment.length) tokenSegments.push(currentTokenSegment.join(' '))
  const labelEvery = mode === 'day' ? 4 : mode === 'week' ? 1 : 5

  return <div className="trend-chart-wrap">
    <svg className="trend-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="网关真实调用次数与 Token 使用量折线图">
      ...
      {tokenSegments.map((segment, index) => <polyline key={index} className="trend-line trend-line-tokens" points={segment}/>)}
  ...
  const loadTrend = useCallback(async () => {
    ...
    try {
      if (trendMode === 'day') {
        ...
        setTrend(hourly.items.map(item => ({ label: item.label, calls: item.calls, tokens: item.total_tokens })))
      } else {
        ...
        setTrend(series.items.map(item => ({ label: item.date.slice(5), calls: item.calls, tokens: item.tokens.total_tokens })))
      }
      setTrendError('')
    } catch (reason) {
      setTrend([])
