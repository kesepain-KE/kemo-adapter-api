  ...
  return <>
    ...
    {!visible.length ? <EmptyState title="没有匹配的模型" description="先匹配网关完整模型名，未命中时再匹配厂商原始模型名。"/> : <div className="model-grid enhanced-model-grid">{visible.map(model => {
      ...
      return <Card key={model.id} className={`enhanced-model-card ${!enabled ? 'model-disabled' : ''}`}>
        <div className="model-head"><div className="model-copy"><strong title={model.id}>{model.id}</strong></div><button className={`toggle ${enabled ? 'on' : ''}`} role="switch" aria-checked={enabled} disabled={providerDisabled || busy === model.id} onClick={() => void toggle(model.id)} aria-label={`${enabled ? '禁用' : '启用'} ${model.id}`}><i/></button></div>
        <div className="model-facts"><div><span>厂商</span><strong title={model.providerId}>{model.providerId}</strong></div><div><span>原始模型名</span><strong title={model.upstreamName ?? 'Provider 未声明'}>{model.upstreamName ?? '未声明'}</strong></div><div><span>任务</span><strong>{model.capability?.task ?? '未声明'}</strong></div></div>
        <div className="model-capabilities"><span>模型能力</span>{tags.length ? <div className="tags">{tags.map(value => <span key={value}>{value}</span>)}</div> : <p>暂无能力声明</p>}</div>
