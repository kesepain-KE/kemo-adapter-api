...
| 修改公开 LLM API | `api.md`、`ADD_DIY/verification.md` | `api/`、`core/models.py` |
...
3. Provider 包只能输出 `ProviderResult`、`ProviderEvent`、`Usage` 和统一错误，不能生成 SSE
   sequence、event_id 或 HTTP Response。
4. `.env`、真实 API Key、Bearer Token、签名 URL 和 Provider State 明文不得写入源码、文档、
   测试快照、日志或 Git。
5. 智能体不得通过正文 `metadata.user` 决定租户或资源授权，必须使用认证后的 Principal。
6. `api.md` 只记录网关对外提供的 LLM/Asset API，不记录 Web 管理端内部接口。
